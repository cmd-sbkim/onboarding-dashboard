...
const SCORE_LABELS = ["", "매우 아니다", "아니다", "보통이다", "그렇다", "매우 그렇다"];
...
function HRApp() {
  ...
  const deletePerson = async (id) => {
    const { error: surveyErr } = await supabase.from('surveys').delete().eq('person_id', id);
    if (surveyErr) { alert('설문 삭제 실패: ' + surveyErr.message); return; }
    const { error } = await supabase.from('people').delete().eq('id', id);
    if (error) { alert('삭제 실패: ' + error.message); return; }
    setData(prev => prev.filter(p => p.id !== id));
    setSurveys(prev => prev.filter(s => s.person_id !== id));
  };
